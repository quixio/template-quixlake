"""
Blob Storage Abstraction Layer using quixportal

This module provides a unified interface to blob storage providers (Azure, AWS S3, GCP, MinIO)
through the quixportal library, replacing direct boto3 S3 client usage.
"""
...
from quixportal.storage.config import load_config_from_env, BlobStorageProvider
...
class BlobStorageClient:
                    ...
                    for f in files:
                        ...
                        try:
                            if self._fs.exists(f):
                                ...
                                result.append({"Key": f, "Size": size if size is not None else 0})
                                found_files.add(f)
    ...
    def put_object_async(self, key: str, body: bytes, callback: Optional[Callable] = None) -> concurrent.futures.Future:
        """
