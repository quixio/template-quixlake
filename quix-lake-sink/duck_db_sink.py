...
class DuckDbSink(BatchingSink):
    ...
    def _append_batch(self, batch: SinkBatch):
        ...
        # 1) Ensure all columns exist in DuckDB table BEFORE creating Arrow table
        ...
        # 4) Get the updated schema AFTER ensuring columns exist
        schema_info = self.con.execute(f"PRAGMA table_info('{self.table_name}')").fetchall()
        ordered_cols = [ row[1] for row in schema_info ]

        # 5) Only select columns that exist in both the table and the tmp_batch
        table_columns = [col.name for col in table.schema]
        available_cols = [col for col in ordered_cols if col in table_columns]
        
        col_list_sql = ", ".join(available_cols)
        select_list_sql = ", ".join(f"COALESCE({col}, NULL) as {col}" for col in available_cols)

        # Now run the INSERT with only available columns
        self.con.execute(f"""
